...
refused it, the token is refreshed first (Codex, Gemini and Kimi; the other
agents renew their own) and the limits follow. When a refresh cannot help
— it failed, the agent's service has ended the session, or the agent renews its own
tokens — `r` asks "Log in again?" right there, and yes runs the login the way `n`
